...
def Удаление_всех_лотов(МаксКолУдаляемыхЛотовПоРесурсу=2):
    ...
    for Resource_for_sale in ('Гб', 'минуты'):
        ...
        if xpath_last_element_html == xpath['Текст_нет_активных_лотов']:
            ...
            СохранитьЗначениеКлючаРеестра(Current_phone+'Выставлное_количествo_Гб', 0, winreg.REG_DWORD)
    ...
    for СпособПолученияОстатков in СпособыПолученяОстатков:
        ...
        for Resource_for_sale in ['минуты', 'Гб']:
            ...
            if isXpath(xpath[f'Остатки_{СпособПолученияОстатков}_{Resource_for_sale}']):
                ...
                if Resource_for_sale == 'Гб' and isXpath('//a[contains(text(),"Подключить пакет Гба")]'):
                    ОстакиПрочитаны = True
...
log_start()
log('Начало')
var ={'Min_count_for_sale_Гб':5, 'Min_count_for_sale_минуты':100, 'Максимальное_увеличение_количества_на_продажу':0,
      'Стоимость_1_ед_Гб':15, 'Стоимость_1_ед_минуты':0.8,
       'Пауза_между_выставлений_лотов_сек':500,
      "Откуда_дарить_Гб":["9535248000", "9012998490"], 'Кому_дарить_Гб':[],
      "Телефоны":{
        # '9517334676': {'Активная_симка': True, 'Name': 'Данила П3т', 'Оставлять_минуты': 600, 'Оставлять_Гб': 0, 'Оставлять_СМС': 100, 'День_обновления_остатков': 16, 'Абонплата': -400, 'Описание': 'Вологда 20 ГБ, 500 минут 500 смс'}

      #     '9535248000': {'Активная_симка': True, 'Name': 'Александр', 'Оставлять_минуты': 600, 'Оставлять_Гб': 15, 'День_обновления_остатков': 17, 'Абонплата': -400, 'Описание': 'Вологда 25ГБ, 600 минут', 'ЛС': '9535248000'},
      #     '9005093361': {'Активная_симка': True, 'Name': 'Александр', 'Оставлять_минуты': 600, 'Оставлять_Гб': 1, 'День_обновления_остатков': 12, 'Абонплата': -500, 'Описание': 'Вологда 35 ГБ, 700 минут', 'ЛС': '9535248000'},
      #     '9005424360': {'Активная_симка': True, 'Name': 'Александр', 'Оставлять_минуты': 0, 'Оставлять_Гб': 0, 'День_обновления_остатков': 12, 'Абонплата': -250, 'Описание': 'Вологда 5 ГБ, 200 минут', 'ЛС': '9535248000'},
      #     '9005033469': {'Активная_симка': True, 'Name': 'Александр', 'Оставлять_минуты': 0, 'Оставлять_Гб': 20, 'День_обновления_остатков': 19, 'Абонплата': -500, 'Описание': 'Вологда 35 ГБ, 700 минут', 'ЛС': '9535248000'},
      #
      '9010452940':{'Активная_симка':True, 'Name':'Надежда модем', 'Оставлять_минуты':0, 'Оставлять_Гб':25, 'День_обновления_остатков':11, 'Абонплата':-300, 'Описание':'40 ГБ, 800 минут', 'Подписаться':True, 'ЛС': '9011720400'},
      # '9011720400':{'Активная_симка':False, 'Name':'Надежда личный', 'Оставлять_минуты':700, 'Оставлять_Гб':35, 'День_обновления_остатков':12, 'Абонплата':-125, 'Описание':' 35 ГБ, 600 минут', 'Подписаться':True, 'ЛС': '9011720400', "Поддерживать_Гб":1},
      # '9011720401':{'Активная_симка':False, 'Name':'Надежда мама', 'Оставлять_минуты':700, 'Оставлять_Гб':5, 'День_обновления_остатков':12, 'Абонплата':-125, 'Описание':' 35 ГБ, 600 минут', 'Подписаться':True, 'ЛС': '9011720400'},
      #
      # '9005034564': {'Активная_симка': True, 'Name': 'Евгения', 'Оставлять_минуты': 500, 'Оставлять_Гб': 25, 'День_обновления_остатков': 9, 'Абонплата': -337.5, 'Описание': 'Вологда  35 ГБ, 600 минут', 'ЛС': ''},
      #
      '9012998490': {'Активная_симка': True, 'Name': 'Александр', 'Оставлять_минуты': 0, 'Оставлять_Гб': 25, 'День_обновления_остатков': 0, 'Абонплата': -300, 'Описание': 'Ярославль 40 ГБ, 800 минут'}
      #
      # '9535202353': {'Активная_симка': True, 'Name': 'Руслан', 'Оставлять_минуты': 500, 'Оставлять_Гб': 5, 'День_обновления_остатков': 20, 'Абонплата': -400, 'Описание': 'Вологда  25ГБ,  600 минут', 'ЛС': ''},
      #
      # '9005507677':{'Активная_симка':True, 'Name':'Орлов Лакомка', 'Оставлять_минуты':0, 'Оставлять_Гб':9, 'День_обновления_остатков':25, 'Абонплата':-500, 'Описание':'Вологда 25ГБ, 350 минут', 'ЛС': '9005072994'},
      # '9005491033':{'Активная_симка':False, 'Name':'Орлов Стекольный', 'Оставлять_минуты':0, 'Оставлять_Гб':9, 'День_обновления_остатков':0, 'Абонплата':-200, 'Описание':'Вологда ГБ, минут', 'ЛС': '9005072994'},
      # '9535208906':{'Активная_симка':True, 'Name':'Орлов Универмаг', 'Оставлять_минуты':0, 'Оставлять_Гб':9, 'День_обновления_остатков':25, 'Абонплата':-500, 'Описание':'Вологда 25ГБ, 600 минут +', 'ЛС': '9005072994'},
      # '9005072994':{'Активная_симка':False, 'Name':'Орлов Личный', 'Оставлять_минуты':0, 'Оставлять_Гб':4, 'День_обновления_остатков':25, 'Абонплата':0, 'Описание':'Вологда 25ГБ, 350 минут', 'ЛС': '9005072994'}
      ...
      # '9005034564':{'Активная_симка':True, 'Name':'Евгения', 'Оставлять_минуты':500, 'Оставлять_Гб':25,'День_обновления_остатков':0}
      }
      }
init_Ветка_Реестра()
...
xpath={'xpath_phone':'//input[@id="keycloakAuth.phone"]', 'xpath_next_login' : '//button[contains(text(),"Далее")]', 'xpath_login_with_password' : '//button[text()="Вход по паролю"]',
      'Текст_нет_активных_лотов':'//div[contains(text(), "У вас нет активных лотов в продаже.")]',
       ...
       # <div>Превышение лимита на размещение лотов в день. Повторите попытку завтра.</div>   //div[contains(text(), "Подтвердить")]
       }
...
for Current_phone in var["Телефоны"].keys():
    ПродаватьС = var["Телефоны"][Current_phone]['ПродаватьС'] = ПолучитьЗначениеКлючаРеестра(Current_phone+'ПродаватьС', "") #0, float
    # if ПродаватьС == 0 :
    #     ПродаватьС = ''
    # else:
...
while True:
    ...
    for Current_phone in var["Телефоны"].keys():
        Активность, ПродаватьС=var["Телефоны"][Current_phone]['Активная_симка'] , var["Телефоны"][Current_phone].get('ПродаватьС', '')
        log(f'Начало переключения на номер {Current_phone} {var["Телефоны"][Current_phone]["Name"]} Активность:{Активность} использовать с: {ПродаватьС} ОбновитьОстатки={var["Телефоны"][Current_phone]["ОбновитьОстатки"]}')
        Активность = (Активность and (_today > ПродаватьС))
        ...
        if  not Активность and not var["Телефоны"][Current_phone]['ОбновитьОстатки'] and not ЕстьКомуДаритьГб:
            continue
        ...
        if (((var["Телефоны"][Current_phone]['Остатки_Гб']-var["Телефоны"][Current_phone]['Оставлять_Гб']) < var['Min_count_for_sale_Гб']) or (var["Телефоны"][Current_phone]['Оставлять_Гб'])==-1) \
            ...
            if var["Телефоны"][Current_phone].get('День_обновления_остатков', 0) == 0:
                ...
                var["Телефоны"][Current_phone]['ПродаватьС'] = ПродаватьС = str(ДатаОбновленияБаланса)[:10] #int(time.mktime(ДатаОбновленияБаланса.timetuple()))
                СохранитьЗначениеКлючаРеестра(Current_phone+'ПродаватьС', ПродаватьС) # , winreg.REG_DWORD
                log(f'Новая дата ПродаватьС для {Current_phone}: {ПродаватьС}') #time.ctime(ПродаватьС) .strftime("%d.%m.%Y")
    ...
    if ВсегоНомеровСПродажей==0:
        ...
        for Current_phone in var["Телефоны"].keys():
            ПродаватьС = var["Телефоны"][Current_phone].get('ПродаватьС', '')
            if var["Телефоны"][Current_phone].get('Активная_симка', False) and near_data > ПродаватьС > '':
        ...
        time.sleep((datetime.datetime.strptime('near_data', '%Y-%m-%d')-datetime.datetime.now()).total_seconds())
        Перезапуск_Браузера(ГлаыныйНомер+'.Profiles_Firefox')
    ...
    for ЛС in Список_ЛС.keys():
        ...
        if  0 < ИзлишекБаланса:
            text = '!!!! Недостаточный баланс {abs(ИзлишекБаланса)} для ' +  text
        log(text)


    log(f'Пауза {var["Пауза_между_выставлений_лотов_сек"]} сек. ВсегоНомеровСПродажей={ВсегоНомеровСПродажей}')
