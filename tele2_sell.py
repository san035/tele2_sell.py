...
while True:
    ...
    for Current_phone in var["Телефоны"].keys():
        Активность, ПродаватьС=var["Телефоны"][Current_phone]['Активная_симка'] , var["Телефоны"][Current_phone].get('ПродаватьС',0)
        log(f'Начало переключения на номер {Current_phone} {var["Телефоны"][Current_phone]["Name"]} Активность:{Активность} использовать с: {time.strftime("%Y-%m-%d", time.gmtime(ПродаватьС))} ОбновитьОстатки={var["Телефоны"][Current_phone]["ОбновитьОстатки"]}')
        Активность = (Активность and (time.time() > ПродаватьС))
        ...
        if (((var["Телефоны"][Current_phone]['Остатки_Гб']-var["Телефоны"][Current_phone]['Оставлять_Гб']) < var['Min_count_for_sale_Гб']) or (var["Телефоны"][Current_phone]['Оставлять_Гб'])==-1) \
            ...
            if var["Телефоны"][Current_phone].get('День_обновления_остатков', 0) == 0:
                ...
                log(f'Новая дата ПродаватьС для {Current_phone} {ДатаОбновленияБаланса.strftime("%d.%m.%Y")}') #time.ctime(ДатаОбновленияБаланса_int)
